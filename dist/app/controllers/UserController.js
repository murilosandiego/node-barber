...
class UserController {
  ...
  async update(req, res) {
    ...
    const { email: emailReq, oldPassword } = req.body;

    const user = await _User2.default.findByPk(req.userId);

    if (emailReq && emailReq !== user.email) {
      const userExist = await _User2.default.findOne({ where: { emailReq } });
    ...
    const { id, name, email, avatar } = await _User2.default.findByPk(req.userId, {
      include: [
        {
